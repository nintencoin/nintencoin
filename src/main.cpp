...
uint256 hashGenesisBlock("0x1066e37dd26ec553292dead5722cf9ce03550cdf4cc4178e13aaef381628557a");
...
const string strMessageMagic = "Nintencoin Signed Message:\n";
...
{
    // Nintencoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if (nHeight > 1 && nHeight <= 1576810) {
        nSubsidy = 31709 * COIN;
    }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nintencoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nintencoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Nintencoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1066e37dd26ec553292dead5722cf9ce03550cdf4cc4178e13aaef381628557a");
    }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Nintencoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NintencoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NintencoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NintencoinMiner(CWallet *pwallet)
{
    printf("NintencoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nintencoin-miner");
    ...
    try { loop {
        ...
        printf("Running NintencoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NintencoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NintencoinMiner, pwallet));
}
